...
pub fn load_protocol_fixture(path: &Path) -> ProtocolParityFixture {
    let contents = fs::read_to_string(path).expect("fixture file should be readable");
    serde_json::from_str(&contents).expect("fixture file should deserialize")
}
...
mod tests {
    ...
    #[test]
    fn protocol_fixture_json_matches_typescript_snapshots() {
        let fixture_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../shared/gameplay-fixtures/protocol/socket-transport.json");
        let fixture = load_protocol_fixture(&fixture_path);

        let client_json =
            serde_json::to_string(&fixture.client_control_message).expect("serialize client");
        let room_json =
            serde_json::to_string(&fixture.server_room_message).expect("serialize room event");
        let callback_json =
            serde_json::to_string(&fixture.server_callback_message).expect("serialize callback");
