...
describe("Role Mechanics and Interactions", () => {
  ...
  class Doctor extends TestRole {
    constructor() {
      super(RoleName.Doctor, RoleGroup.Town);
  ...
  class Mafia extends TestRole {
    constructor() {
      super(RoleName.Mafia, RoleGroup.Mafia);
      this.nightVote = true;
      this.damage = 1;
    }
  ...
  class Investigator extends TestRole {
    ...
    constructor() {
      super(RoleName.Investigator, RoleGroup.Town);
      this.nightVisitOthers = true;
    }
  ...
  class Bodyguard extends TestRole {
    constructor() {
      super(RoleName.Bodyguard, RoleGroup.Town);
      this.nightVisitOthers = true;
      this.damage = 1;
    }
  ...
  class Maniac extends TestRole {
    constructor() {
      super(RoleName.Maniac, RoleGroup.Neutral);
      this.nightVisitOthers = true;
      this.damage = 1;
    }
  ...
  class Roleblocker extends TestRole {
    constructor() {
      super(RoleName.Roleblocker, RoleGroup.Town);
      this.nightVisitOthers = true;
    }
  ...
  describe("Basic Role Properties", () => {
    it("should create roles with correct basic properties", () => {
      ...
      expect(doctor.name).toBe(RoleName.Doctor);
      expect(doctor.group).toBe(RoleGroup.Town);
      expect(doctor.nightVisitOthers).toBe(true);

      expect(mafia.name).toBe(RoleName.Mafia);
      expect(mafia.group).toBe(RoleGroup.Mafia);
      expect(mafia.nightVote).toBe(true);
      expect(mafia.damage).toBe(1);

      expect(investigator.name).toBe(RoleName.Investigator);
      expect(investigator.group).toBe(RoleGroup.Town);
  ...
  describe("Investigator Mechanics", () => {
    it("should investigate target and get results", () => {
      ...
      expect(result).toBe(
        `${RoleName.Mafia} is a member of ${RoleGroup.Mafia}`,
      );
    ...
    it("should fail to investigate when roleblocked", () => {
      ...
      expect(investigator.investigationResults).not.toContain(
        `${RoleName.Mafia} is a member of ${RoleGroup.Mafia}`,
      );
    ...
    it("should track multiple investigation results", () => {
      ...
      expect(investigator.investigationResults[0]).toContain(RoleGroup.Mafia);
      expect(investigator.investigationResults[1]).toContain(RoleGroup.Town);
    });
  });
  ...
  describe("Complex Role Interactions", () => {
    ...
    it("should handle investigator finding mafia", () => {
      ...
      expect(result).toContain(RoleGroup.Mafia);
      ...
      roles.forEach((role) => {
        ...
        if (role.nightVisitOthers) {
          expect(role.name).toMatch(
            /Doctor|Investigator|Bodyguard|Maniac|Roleblocker/,
          );
        }

        // Mafia should have night vote
        if (role.group === RoleGroup.Mafia) {
