...
- `RoleDefinition` describes stable catalog data: metadata, combat baseline, capabilities, traits, and ordered `RoleHandler` entries.
...
- `RoleHandler` implementations are executable behavior. They should use the composition helper modules for command results, targeting, effects, notices, and persistent runtime slots instead of mutating ad hoc closure state.
...
- Built-in and custom roles both normalize through `createRoleInstance()`, then share the same handler/runtime/validation path.
