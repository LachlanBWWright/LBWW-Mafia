...
export function AccountSettings({
  ...
  const [stats, setStats] = useState<PlayerStats | undefined>(initialStats);
  const [message, setMessage] = useState("");
  useEffect(() => {
    if (initialProfile) return;
    void Promise.all([
      client.account.profile.query(),
      client.account.stats.query(),
    ]).then(([p, s]) => {
      setProfile(p);
      setStats(s);
    });
  ...
  async function save(formData: FormData) {
    ...
    const updated = await client.account.updateProfile.mutate({
      name: field("name"),
      handle: field("handle"),
      bio: field("bio"),
      profileVisibility: field("profileVisibility") as
        | "public"
        | "friends"
        | "private",
      historyVisibility: field("historyVisibility") as
        | "public"
        | "friends"
        | "private",
      theme: field("theme") as "dark" | "light" | "system",
        ...
        <CardContent>
          <form action={save} className="grid gap-4 sm:grid-cols-2">
            ...
            <SelectField
              ...
              value={profile.historyVisibility}
              options={["public", "friends", "private"]}
            />
            <SelectField
              name="theme"
